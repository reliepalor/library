...
<body class="bg-gray-100 font-sans antialiased">
    ...
    <div class="flex flex-col items-center justify-center min-h-screen py-12 px-4">
        ...
        <div class="bg-blue-50 rounded-2xl shadow-sm p-8 w-[100vw] max-w-md" x-data="{ showQrModal: false }">
            ...
            @if ($errors->any())
                <div class="mb-6 p-4 bg-red-50 text-red-700 rounded-lg border border-red-200 transition-opacity duration-500" 
            ...
            <div class="text-center mt-8">
                <h1 class="text-2xl font-bold text-gray-900 mb-2">Change Profile Picture</h1>
                ...
                <div class="relative group mb-6">
                    ...
                    <img id="profile-preview" 
                         ...
                         class="w-40 h-40 rounded-full object-cover mx-auto ring-4 ring-blue-500 shadow-lg transition-transform duration-300 ease-in-out group-hover:scale-105" />

                    <!-- Upload button overlay -->
                    <label for="profile_picture" 
                           class="absolute bottom-2 right-2 p-2 bg-blue-600 text-white rounded-full cursor-pointer hover:bg-blue-700 shadow-lg transition transform hover:scale-110">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data" 
                      class="space-y-6" id="profile-picture-form">
                    @csrf
                    @method('PATCH')
                    ...
                    <!-- File name display -->
                    <div id="file-info" class="text-sm text-gray-600 text-center hidden">
                    ...
                    <button type="submit" 
                            id="upload-btn"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg shadow-md transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled>
                    ...
                    <a href="{{ route('user.profile.edit') }}" 
                       class="block w-full text-center bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-3 px-4 rounded-lg transition duration-300">
                        Cancel
                    </a>
                </form>
            </div>
        ...
        @if ($user->student)
            <div class="bg-gray-50 rounded-xl shadow p-6 sm:p-8 space-y-6 mt-8 max-w-4xl w-full">
                <h3 class="text-xl font-semibold text-gray-800 border-b border-gray-200 pb-2">Student Profile Information</h3>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                    <div class="bg-white p-4 rounded-lg shadow-sm transition transform hover:scale-[1.02] hover:shadow-md">
                ...
                @if($user->student && $user->student->qr_code_path)
                    <div class="border-t pt-6 mt-6 flex justify-center flex-col items-center">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Your QR Code</h3>
                        <p class="text-sm text-gray-600 mb-4">Click to view full size</p>

                        <div x-data="{ showQrModal: false }" class="relative">

                            <!-- Existing QR code image -->
                            <img src="{{ asset('storage/' . $user->student->qr_code_path) }}" 
                                 alt="Student QR Code"
                                 class="w-64 h-64 object-contain border border-gray-200 rounded-lg shadow-md cursor-pointer hover:scale-105 transform transition-transform duration-300"
                            ...
                            <div x-show="showQrModal"
                                ...
                                <div class="bg-white rounded-2xl p-6 relative max-w-lg w-full shadow-2xl transform transition-all"
                                    @click.stop
                                    x-show="showQrModal"
                                    x-transition:enter="transition ease-out duration-300"
                                    ...
                                    <button @click="showQrModal = false" 
                                            class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 focus:outline-none transition-colors duration-200 z-10">
                                    ...
                                    <div class="text-center">
                                        <h3 class="text-xl font-semibold text-gray-900 mb-4">Your Student QR Code</h3>
                                        <div class="flex justify-center">
                                            <img src="{{ asset('storage/' . $user->student->qr_code_path) }}"
                                                 alt="Student QR Code"
                                                 class="w-64 h-64 object-contain border border-gray-200 rounded-lg shadow-md" />
                                        </div>
                                        <p class="text-sm text-gray-600 mt-4">Use this QR code for attendance and library services</p>
                                    </div>
                                </div>
                            </div>
