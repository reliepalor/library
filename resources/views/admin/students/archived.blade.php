...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Admin | Archived Students</title>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 text-gray-900">
                            <div class="flex justify-between items-center mb-6">
                                <h2 class="text-2xl font-semibold">Archived Students</h2>
                                <a href="{{ route('admin.students.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white text-sm font-medium rounded-md hover:bg-gray-700 transition">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach($students as $student)
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $student->college }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $student->year }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $student->archived_at->format('M d, Y H:i') }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        <form action="{{ route('admin.students.unarchive', $student->id) }}" method="POST" class="inline">
                                                            ...
                                                            <button type="submit" 
                                                                ...
                                                                onclick="return confirm('Are you sure you want to unarchive this student?')"
                                                                title="Unarchive">
