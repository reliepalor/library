 // Main JavaScript for admin scan/webcam attendance
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const handleLogout = async (studentId, activity) => {
            ...
            try {
                const response = await fetch('/admin/teachers-visitors-attendance/log', {
                    method: 'POST',
                    headers: {
        ...
        const handleQrScan = async (qrData) => {
            ...
            try {
                // For teachers/visitors, QR code format is: teacher_visitor_id|name|department|role
                const cleanData = qrData.replace(/[^-|]/g, '').trim();
                const parts = cleanData.split('|').map(part => part.trim());
                if (parts.length !== 4) throw new Error('Invalid QR code format. Expected teacher/visitor ID, name, department, and role.');
                const [teacherVisitorId, name, department, role] = parts;
                if (!teacherVisitorId || !name || !department || !role) throw new Error('QR code missing required fields.');

                // Check if this looks like a student QR code
                if (role.toLowerCase().includes('year:') || department.toLowerCase().includes('year:')) {
                    throw new Error('This appears to be a student QR code. Please use the student attendance page for student QR codes.');
                }

                const csrfToken = getCSRFToken();
                if (!csrfToken) throw new Error('CSRF token not found. Please refresh the page.');

                // Fetch teacher/visitor data
                const response = await fetch(`/admin/teachers-visitors-attendance/scan?teacher_visitor_id=${encodeURIComponent(teacherVisitorId)}`, {
                    method: 'GET',
                    credentials: 'same-origin',
                    headers: {
                        'Accept': 'application/json',
                ...
                if (!data.teacherVisitor) throw new Error('Teacher/Visitor not found in database');

                // Check for active session
                const checkResponse = await fetch(`/admin/teachers-visitors-attendance/check?teacher_visitor_id=${encodeURIComponent(teacherVisitorId)}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
                ...
                if (checkData.hasActiveSession) {
                    // Rescan while active => perform logout with confirmation UI
                    await handleLogout(teacherVisitorId, checkData.activity);
                    return;
                }

                // If not logged in, show the activity modal for login
                modalStudentId.value = teacherVisitorId; // Reuse the same field for teacher_visitor_id
                const nameParts = name.split(' ');
                const fname = nameParts[0] || '';
                const lname = nameParts.slice(1).join(' ') || '';
                const teacherVisitorName = data.name || `${fname} ${lname}`;
                const profilePicUrl = generateAvatarUrl(teacherVisitorName, 100); // Use avatar for teachers/visitors

                const teacherVisitorDetailsHtml = `
                    <div class="flex flex-col">
                        <p class="text-lg font-semibold text-gray-900">${lname}, ${fname}</p>
                        <div class="mt-1 flex flex-wrap items-center gap-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 ring-1 ring-gray-200">ID: ${teacherVisitorId}</span>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 ring-1 ring-blue-200">${department}</span>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700 ring-1 ring-green-200">${role}</span>
                        </div>
                    </div>
                `;
                ...
                if (profilePicImg) {
                    ...
                    profilePicImg.onerror = function() {
                        this.onerror = null;
                        this.src = generateAvatarUrl(teacherVisitorName, 100);
                    };
                }
                ...
                studentDetailsDiv.innerHTML = teacherVisitorDetailsHtml;

                // Update modal labels for teachers/visitors
                const modalTitle = document.querySelector('#activity-modal h3');
                if (modalTitle) modalTitle.textContent = 'Select Activity for Teacher/Visitor';
                ...
                // Set default activity for teachers/visitors
                activitySelect.value = 'Visit';

                activityModal.classList.remove('hidden');
                showStatus('Teacher/Visitor found! Please select activity for login.', 'success');
            } catch (error) {
